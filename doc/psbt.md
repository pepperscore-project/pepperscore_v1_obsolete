# PSBT Howto for PeppersCore Core

Since PeppersCore Core 0.17, an RPC interface exists for Partially Signed PeppersCore
...
PSBT is an interchange format for PeppersCore transactions that are not fully signed
...
Overall, the construction of a fully signed PeppersCore transaction goes through the
following steps:
...
- An **Extractor** produces a valid PeppersCore transaction (in network format)
...
#### Multisig with multiple PeppersCore Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
PeppersCore Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `pepperscore-cli` is used.
