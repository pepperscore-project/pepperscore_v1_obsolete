...
const char * const BITCOIN_CONF_FILENAME = "pepperscore.conf";
...
#else
    const char* pszModule = "pepperscore";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PeppersCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PeppersCore";
#else
    // Unix
    return pathRet / ".pepperscore";
#endif
#endif
}
